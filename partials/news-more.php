<div id="nov-ost" ng-controller="ucitajNovost">
	<div class="novost">
		<div class = "header">
			<?=$data[0]->naslov;?>
		</div>
        
        <div id="ikonice">

            <!--<a href=""><img id="google" alt="+google" src="img/social-media/google.png"></a>-->
            <a href="https://www.linkedin.com/cws/share?url=http//www.jobfair.ba/novost/<?=$data[0]->id;?>"><img id="linkedin" alt="linked" src="<?=$url_home;?>img/social-media/linkedin.png"></a>
            <a href="https://twitter.com/intent/tweet"><img id="twitter" alt="tviter" src="<?=$url_home;?>img/social-media/twitter.png"></a>
            <a href="https://www.facebook.com/sharer/sharer.php?u=http//www.jobfair.ba/novost/<?=$data[0]->id;?>"><img id="facebook" alt="fejs" src="<?=$url_home;?>img/social-media/facebook.png"></a>
            <br>
        </div>

        <div align = "">
<!--            Samo mijenjamo broj galerije kao parametar f-je-->
            <div class="gallery-news" style="position:relative;">
                <?php if(count($galerija) > 0): ?>
                    <div class="otvori-galeriju">
    					<img class="icon icons8-Stack-of-Photos" src="data:image/png;base64," width="200" height="200">
                        <br />
                        Otvori galeriju
                    </div>
                <?php endif; ?>
            		<div class="news-img-top">
                    <div class="slika-news-gallery" style= "background: url('<?=$data[0]->velika_slika;?>') no-repeat center bottom; background-size:cover;" > </div>
                        <?php if(count($galerija) > 0): ?>
                            <?php foreach($galerija as $item){ ?>
                                <a class="grouped-elements" rel="gal2" style="position:absolute;top:0;left:0;right:0;bottom:0;" href="<?=$item;?>"></a>
                            <?php } ?>
                        <?php endif; ?>
                </div>
            </div>
            <br><br>
            <div id ="text">
								<b style="font-size:18px;"><?=$data[0]->opis;?></b>

                <br><br>

                <?=$data[0]->sadrzaj;?>
            </div>
            <script>$(document).ready(function(){ $("a.grouped-elements").fancybox(); });</script>
<!--        
            <br><br>
            <div class = "slike">
                <li><img src = "img/galerija/2.JPG" alt="" height = "100px" width = "200px"/></li>
                <li><img src = "img/galerija/1.JPG" alt="" height = "100px" width = "200px"/></li>
                <li><img src = "img/galerija/2.JPG" alt="" height = "100px" width = "200px"/></li>
                <li><img src = "img/galerija/1.JPG" alt="" height = "100px" width = "200px"/></li>
                <li><img src = "img/galerija/1.JPG" alt="" height = "100px" width = "200px"/></li>
                <li><img src = "img/galerija/2.JPG" alt="" height = "100px" width = "200px"/></li>
                <li><img src = "img/galerija/1.JPG" alt="" height = "100px" width = "200px"/></li>
                <li><img src = "img/galerija/2.JPG" alt="" height = "100px" width = "200px"/></li>
                <li><img src = "img/galerija/2.JPG" alt="" height = "100px" width = "200px"/></li>
                <li><img src = "img/galerija/1.JPG" alt="" height = "100px" width = "200px"/></li>
                <li><img src = "img/galerija/2.JPG" alt="" height = "100px" width = "200px"/></li>
                <li><img src = "img/galerija/1.JPG" alt="" height = "100px" width = "200px"/></li>
            </div>
-->
        </div>
    </div>
</div>
